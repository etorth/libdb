...
`mir2x` features a built-in Pinyin input method, providing out-of-the-box convenience for systems lacking a native Chinese input method. This feature is powered by `libpinyin`, which depends on Berkeley DB. While `libpinyin` can alternatively use Kyoto Cabinet, that alternative also requires patching.

When using `vcpkg` as the project's build system, we encounter two main limitations:

1. **Platform Restrictions:** The official `vcpkg` port for Berkeley DB only supports Windows (MSVC), whereas `mir2x` targets both Linux (Native/WSL2) and MinGW UCRT64.
2. **Patching Requirements:** Using the upstream [Berkeley DB repository](https://github.com/berkeleydb/libdb.git) as an overlay port requires custom patching to compile successfully on MinGW.
...
1. Open a **MinGW UCRT64** terminal.
2. Clone the MSYS2 packages repository:
```bash
...
3. Navigate to the Berkeley DB package directory:
```bash
...
4. Download and patch the source code without running the full compilation:
```bash
makepkg-mingw -o
